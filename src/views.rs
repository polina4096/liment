...
use objc2_app_kit::{
  NSColor, NSFont, NSLayoutConstraint, NSProgressIndicator, NSProgressIndicatorStyle, NSTextField, NSView,
};
use objc2_core_foundation::CGFloat;
use objc2_foundation::{NSArray, NSSize, NSString};

use crate::api::SubscriptionTier;
...
pub fn make_progress_row(mtm: MainThreadMarker, label: &str, utilization: f64, reset_str: &str) -> Retained<NSView> {
...
pub fn make_header_row(mtm: MainThreadMarker, title: &str, tier: Option<SubscriptionTier>) -> Retained<NSView> {
  let container = NSView::init(mtm.alloc::<NSView>());
...
pub fn make_label_row(mtm: MainThreadMarker, text: &str, bold: bool) -> Retained<NSView> {
  let container = NSView::init(mtm.alloc::<NSView>());
...
pub fn make_key_value_row(mtm: MainThreadMarker, key: &str, value: &str) -> Retained<NSView> {
  let container = NSView::init(mtm.alloc::<NSView>());
  container.setFrameSize(NSSize::new(280.0, 22.0));
